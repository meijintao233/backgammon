...
(function initWinningData(){
	...
	for (var i = commonConstant; i <= width - commonConstant; i += positionX) {
		for (var j = commonConstant; j <= height - commonConstant - 4 * positionY; j += positionY) {
			for (var k = 0; k < 5; k++) {
				wins[parseInt(i/positionX)][parseInt(j/positionY) + k][count] = true;
			}
			count++;
		}
	}
	...
	for (var i = commonConstant; i <= width - commonConstant; i += positionX) {
		for (var j = commonConstant; j <= height - commonConstant - 4 * positionY; j += positionY) {
			for (var k = 0; k < 5; k++) {
				wins[parseInt(j/positionY) + k][parseInt(i/positionX)][count] = true;
			}
			count++;
		}
	}
	...
	for (var i = commonConstant; i <= width - commonConstant - 4 * positionX; i += positionX) {
		for (var j = commonConstant; j <= height - commonConstant - 4 * positionY; j += positionY) {
			for (var k = 0; k < 5; k++) {
				wins[parseInt(i/positionX) + k][parseInt(j/positionY) + k][count] = true;
			}
			count++;
		}
	}
	...
	//anti-oblique data
	for (var i = commonConstant; i <= width - commonConstant - 4 * positionX; i += positionX) {
		for (var j = height - commonConstant - positionY; j > commonConstant + 3 * positionY; j -= positionY) {
			for (var k = 0; k < 5; k++) {
				wins[parseInt(i/positionX) + k][parseInt(j/positionY) - k][count] = true;
			}
			count++;
		...
		for (let i = commonConstant; i <= width - commonConstant; i += positionX) {
			for (let j = commonConstant; j <= height - commonConstant; j += positionY) {
				if((pos.x - i) * (pos.x - i) + (pos.y - j) * (pos.y - j) < 100){
					...
					ctx.stroke();				
					ctx.fill();
					data[parseInt(i/positionX)][parseInt(j/positionY)] = 0;
				...
				for (let k = 0; k < count; k++) {
					if(wins[parseInt(i/positionX)][parseInt(j/positionY)][k]){
						if(myWins[k] == 1){
							...
							myScore[parseInt(i/positionX)][parseInt(j/positionY)] += 2000;
						}else if(myWins[k] == 4){
							myScore[parseInt(i/positionX)][parseInt(j/positionY)] += 20000;
						}
						if(AIWins[k] == 1){
							...
							AIScore[parseInt(i/positionX)][parseInt(j/positionY)] += 2500;
						}else if(AIWins[k] == 4){
							AIScore[parseInt(i/positionX)][parseInt(j/positionY)] += 50000;
						}
					}
				}
